...
def generate_rsa_keypair(email: str, passphrase: str, mode="create") -> dict:
...
def update_public_key_store(email: str):
    ...
    public_key_path = f"{PUBLIC_KEY_DIR}/{email}.json"

    # Load current key
    try:
        with open(f"../data/{email}/rsa_keypair.json", "r") as f:
    ...
    # Load archived keys
    try:
        with open(f"../data/{email}/archived_keys.json", "r") as f:
            archived_keys = json.load(f)
